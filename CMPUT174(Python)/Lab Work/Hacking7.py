...
def display_password_list(window, location):
    ...
    for password in password_list:
        password_line = embed_password(password,total_embed)
...
def get_guesses(window, password, location, attempts_left):
    ...
    feedback_loc = [(window.get_width() // 2),0]
    ...
    while guess != password and attempts_left > 0:
        ...
        display_hint(window,password,guess,feedback_loc)
        guess = prompt_user(window, prompt, location)
        attempts_left = attempts_left - 1
    return guess
...
def display_hint(window,password,guess,location):
    ...
    index = 0
    correct = 0
    maximum = len(password)
    for letter in guess:
        if (index < maximum) and (letter == password[index]):
            correct = correct + 1
        index = index + 1
    string = str(correct) + '/' + str(maximum) + ' IN MATCHING POSITIONS'
    display_line(window,string,location)
main()
