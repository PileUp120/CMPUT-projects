...
public class LevelOptimizer
{
    ...
    const int POPULATION_SIZE = 20;//The size of the population
    const float MUTATION_RATE = 0.5f;//The rate at which to mutate population members
    const int NUM_CROSSOVERS = 45;//The number of crossovers/children to produce
    const int MAX_ATTEMPTS = 500;//The max number of times to iterate the algorithm
    ...
    public Level GetLevel(Evaluator evaluator, float goalValue)
    {
        ...
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++)
        {
            ...
            {
                Level level_1 = Sample(population, evaluator, goalValue);
                Level level_2 = Sample(population, evaluator, goalValue);
                while (level_1.Equals(level_2))
                {
                    level_2 = Sample(population, evaluator, goalValue);
                }
                population.Add(Crossover(level_1, level_2));
            }
