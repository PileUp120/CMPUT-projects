...
class GtpConnection:
    def __init__(self, go_engine, board, debug_mode=False):
        ...
        self.board = board
        self.policy = go_engine.random_simulation
        self.selection = go_engine.random_simulation
        self.commands = {
    ...
    def policy_moves_cmd(self, args):
        ...
        """
        policy_type = self.go_engine.random_simulation
        ...
        if(policy_type):
            policy_moves = GoBoardUtil.generate_legal_moves(self.board, self.board.current_player)
            
            if(len(policy_moves) == 0):
                self.respond(colors[GoBoardUtil.opponent(self.board.current_player)])
            probability = 1.0/len(policy_moves)
            probability = round(probability, 3)
            
            letter_moves = []
            for move in policy_moves:
                position = point_to_coord(move, self.board.size)
                position = format_point(position)
                position = position.lower()
                letter_moves.append(position)

            letter_moves = sorted(letter_moves)
            
            response = ""
            for move in letter_moves:
                response += (move + " ")
            
            for i in range(len(policy_moves)):
                response += (str(probability) + " ")
            ...
            probablitiies = self.go_engine.find_pattern_probablilites(self.board, policy_moves)
            letter_probabilities = {}
            for move in probablitiies.keys():
                letter_probabilities[format_point(point_to_coord(move, self.board.size)).lower()] = probablitiies[move]
            ...
            for move in letter_moves:
                response += (str(letter_probabilities[move]) + " ")
