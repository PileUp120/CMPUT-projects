...
class GoBoard(object):
    ...
    def weight_finder(self, point, pattern_list):
        section_one = self._neighbors(point) #[left, right, above, below]
        
        #the functions return indexes of the positions, we want the value instead of the index
        for i in range(len(section_one)):
            section_one[i] = self.get_color(section_one[i])
        
        section_two = self._diag_neighbors(point)
        for j in range(len(section_two)):
            section_two[j] = self.get_color(section_two[j])
        #[top-left, top-right, bottom-left, bottom-right]
        #bottom right, bottom, bottom left, right, left, top-right, top, top-left
        
        patter_number = str(section_two[3])+str(section_one[3])+str(section_two[2])+str(section_one[1])+str(section_one[0])+str(section_two[1])+str(section_one[2])+str(section_two[0])
        
        if patter_number in pattern_list.keys():
            return pattern_list[patter_number]
        else:
            return 1.0
