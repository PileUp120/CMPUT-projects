...
class GoBoardUtil(object):
    ...
    def generate_random_moves(board):
        """
        Return a list of random (legal) moves 
        """
    ...
    @staticmethod
    def get_pattern_list(filename):
        pattern_list = open(filename, 'r').read().splitlines()

        patterns_and_weights = {}

        for pattern in pattern_list:
            pattern_with_weight = pattern.split(' ')
            if (pattern_with_weight[1] == '1.0'):
                continue
            arg = int(pattern_with_weight[0])
            r = 0
            s = ''
            while (len(s) < 8):
                r = arg%4
                s = str(r) + s
                arg //= 4
            patterns_and_weights[s] = float(pattern_with_weight[1])
        
        return patterns_and_weights
