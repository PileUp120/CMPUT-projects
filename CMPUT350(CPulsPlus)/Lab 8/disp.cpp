...
void OnDisplay()
{
  ...
  glLoadIdentity();
  glOrtho(0, triang->get_width(), triang->get_height(), 0, 0, 1);
  ...
  for (const Triangle &tr : triang->get_triangles()) {
    glColor3f(rnd01(col_rng)*0.5 + 0.5,
              rnd01(col_rng)*0.5 + 0.5,
              rnd01(col_rng)*0.5 + 0.5);
    glBegin(GL_TRIANGLES);
    for (size_t i=0; i < 3; ++i) {
      // invert y
      // in OpenGL (0,0) is top left, (width,height) bottom right
      glVertex2f(tr.points[i].x, triang->get_height()-tr.points[i].y);
    }
    glEnd();
  }
...
void timer_function(int)
{
  ...
  int D = 20;
  int x = (triang->get_width() -2*D) * rnd01(rng) + D;
  int y = (triang->get_height()-2*D) * rnd01(rng) + D;

  if (mode) {
    y = (int)((((double)x) / triang->get_width()) * triang->get_height() + 0.5);
    // cout << "x " << x << " y " << y << endl;
    assert(y > 0 && y < (int)triang->get_height());
  }
  ...
  glutTimerFunc(current_delay, timer_function, 0);
}
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 1 && argc != 8) {
    cerr << "usage: " << argv[0] << " width height delay seed mode fspeed num"
         << endl
         << "       delay in ms, seed=0: use time, mode 0/1, flip speed 0/1, number of points"
         << endl;
    exit(1);
  }
