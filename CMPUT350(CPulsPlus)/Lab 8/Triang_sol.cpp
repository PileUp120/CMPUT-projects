...
void Triangulation::add_point(const Point &p)
{
  ...
  if (ins < 0) {
    ...
    */
    ...
    size_t other = t.neighbors[e];

    triangles.push_back(Triangle());
    triangles.push_back(Triangle());

    size_t new_l     = triangles.size()-2;
    size_t new_other = triangles.size()-1;
...
void Triangulation::set_neighbor(size_t neighbor_ind, size_t old_ind,
                                 size_t new_ind)
{
...
void Triangulation::split_triangle(size_t ind, size_t new_ind, const Point &p,
...
bool Triangulation::flip_if_better(size_t t_ind, int e)
{
  ...
  int i;
  for (i=0; i < 3; ++i) {
    if (s.points[i] != t.points[e] &&
        s.points[i] != t.points[(e+1)%3]) {
      break;
    }
  }
