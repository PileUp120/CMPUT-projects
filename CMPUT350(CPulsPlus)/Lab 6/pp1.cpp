...
public:
    ...
    void push(const T& x) { Base::push_back(x); }

    // returns reference to front element (pre-cond: !empty)
    T& front() { assert(!empty()); return Base::front(); }

    // returns const reference to front element (pre-cond: !empty)
    const T& front() const { assert(!empty()); return Base::front(); }

    // removes front element (pred-cond: !empty)
    void pop() { assert(!empty()); Base::pop_front(); }
