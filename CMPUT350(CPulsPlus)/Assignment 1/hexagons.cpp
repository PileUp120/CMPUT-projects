...
void generate_points(double r, double &x, double &y, int i, int j) {
    
    x = random_double(0,1) + (r*(float)(2/3)*i);
    y = random_double(0,1) + (r*sqrt(3)(i/2 + j));
    
}

double random_double(double max, double min){ //generate a randon number in the given range
...
/*
  ...
  Locate function: from looking at the diagram, if a hexagon grid is laid over a rectangular grid, the distance from the center of  one hexagon to the center of the adjacent hexagon is 1 
  along the x axis, and 2 along the y axis. 
  If we form a triangle using the center of three adjacent hexagons, we form a uniform triangle of side length 2, a line that divides the trangle in half from any of it's three 
  vertices has a length 1
  As hexagons have equal side lengths, and the triangle is between 3 hexagons, a vertex of a hexagon lies 2/3 of the way through the line that splits the mentioned triangle in half.
  Hence the length of the sides of a hexagon is 2/3, which is also the distance between a hexagon center and any of it's vertices. Meaning that for any x, a center of a hexagon can
  be found at most 2/3 away from itself. Thus i = (2/3)x/r, to find an appropriately close hexagon center we round it so that if it is less than 0.5 away from an i it goes towards
  that i and if it is 0.5 or more away from that i, it goes towards the next i.
  As for the j, since j is like the y axis if it was rotated 45 degrees to the left, there is now an angle of 30 degress between the j axis and the x axis, and from the explainations about 
  hexagonal grids from the webpages linked in README.txt, j can be found using tan(30)*y + sin(30)*i.
  Since i has already been appropriately rounded, the y values will also have to be rounded so that if it is less than 0.5 from a j it goes towards that j, and if it is 0.5 or more
  from that j, it goes towards the next one.
