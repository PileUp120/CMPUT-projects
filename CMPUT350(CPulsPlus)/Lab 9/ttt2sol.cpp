...
public:
  ...
  */
  ...
  int max_value() const
  {
    ...
    for (int y=0; y < N; ++y) {
      int s = 0;
      for (int x=0; x < N; ++x) { s += sq[y][x]; }
      if (s == N*MAX) { return MAX; }
      if (s == N*MIN) { return MIN; }
    }

    // check columns
    for (int x=0; x < N; ++x) {
      int s = 0;
      for (int y=0; y < N; ++y) { s += sq[y][x]; }
      if (s == N*MAX) { return MAX; }
      if (s == N*MIN) { return MIN; }
    }
    ...
    {
      int s = 0;    
      for (int x=0; x < N; ++x) { s += sq[x][N-1-x]; }
      if (s == N*MAX) { return MAX; }
      if (s == N*MIN) { return MIN; }
    }
...
tuple<int,int,int> negamax(const State &st)
{
  ...
  assert(move_found);
  assert(best_x >= 0 && best_x < State::N &&
         best_y >= 0 && best_y < State::N); // move was found
