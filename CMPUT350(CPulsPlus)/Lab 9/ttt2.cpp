...
{
  InputException(const std::string &reason)
    : reason(reason)
  {}
...
public:
    ...
    */
    ...
    for (int y=0; y < N; ++y) {
      for (int x=0; x < N; ++x) {
        ...
        if      (c == get_disp(MAX))   { sq[y][x] = MAX; ++filled; }
        else if (c == get_disp(MIN))   { sq[y][x] = MIN; ++filled; }
        else if (c == get_disp(EMPTY)) { sq[y][x] = EMPTY; }
        else {
          // invalid symbol
  ...
  */
  ...
  int max_value() const
  {
    ...
    //check colums to see if there is a match
    for (int i = 0; i < N; i++)
      {
        int j = 0;
        while (j < (N-1))
        ...
        if ((j == (N-1)) && (sq[i][j] != EMPTY)) { 
          found_winner = true;
          winner = sq[i][j];
          }
      }
    //check rows to see if there is a match, if no winner found from columns
    if (!found_winner)
    {
      for (int i = 0; i < N; i++)
      {
        ...
        if ((j == (N-1)) && (sq[j][i] != EMPTY)) { 
          found_winner = true;
          winner = sq[j][i];
          }
      }
    }
    //check diagonals to see if there is a match, if no winner found from rows or columns
    ...
    if(!found_winner)
    {
      ...
      while (l < (N-1))
      {
        if (sq[l][l] != sq[l+1][l+1])
        {
          break;
        }
        l++;
      }
      if ((l == (N-1)) && (sq[0][0] != EMPTY)) { 
          found_winner = true;
          winner = sq[0][0];
          }
    }
  ...
  bool make_move(int x, int y)
  {
    ...
    bool move = false;
    if (sq[y][x] == get_disp(EMPTY)) {  move = true; }
    return move;
  }
...
void solve(const State &st)
{
  ...
  if (st.max_value() != 0)
  {
    std::cout << "a player already won" << "\n";
  }
  else if ((st.max_value() == 0) && st.full())
  {
    std::cout << "draw" << "\n";
  }
  else
  {
    auto endgame = negamax(st);
    if (std::get<0>(endgame) == 0)
    {
        std::cout << "p draws with" << "(" << std::get<1>(endgame) << ", " << std::get<2>(endgame) << ")" << "\n";
    }
    else if (std::get<0>(endgame) == 1)
    {
        std::cout << "p wins with" << "(" << std::get<1>(endgame) << ", " << std::get<2>(endgame) << ")" << "\n";
    }
    else if (std::get<0>(endgame) == -1)
    {
        std::cout << "p loses" << "\n";
