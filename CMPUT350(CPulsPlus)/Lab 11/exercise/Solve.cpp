...
void solve(Matrix &A)
{
    ...
    
  {
    ...
    */
    ...
    for(int j = 0; j < N2; j++)
    {
      fprintf(fp, "Z+%f ",min_A);

      for(int i = N1; i > 0; i--)
      {
        fprintf(fp, "- %f x%d", (min_A+(N1-i)+j), (N1-i));
      }
    ...
    { int res = system("lp_solve < p1.lp > p1.res"); (void)res; } // make lab g++ happy
...

  {
    ...
    */
    ...
    for(int i = 0; i < N1; i++)
    {
      ...
      for(int j = N2; j > 0; j--)
      {
        fprintf(fp, "- %f x%d", (min_A+i+(N2-j)), (N2-j));
      }
    ...
    // launch external LP solver    
    { int res = system("lp_solve < p2.lp > p2.res"); (void)res; } // make lab g++ happy    
...
void best_response_to_col(const Matrix &A, const Vector &strat2)
{
   ...
   */
  assert(A.size() > 0);
  int C1 = A[0].size(); //number of columns
  int R1 = A.size(); //number of rows

  assert((int)strat2.size() == C1); //make sure that there are enough columns in the payoff matirx to find a response to all column strategies

  double min_val = 0; //neutral response
  int min_ind = -1; 

  for (int i=0; i < R1; ++i) { //for every row

    double sum = 0;

    for (int j=0; j < C1; ++j) { //calculate the sum for every column to see if it is in favor of row player
      sum += strat2[j] * A[j][i];
    }
