/*
  ...
  tools you used: Moved the line CoolBear ziggy = getCoolbearCBList( cblist, 0) 
  to after the addCoolBearCBList function loop, since the function uses realloc,
  so the pointer to the address of ziggy is changed(tested by printing the
  pointer address of ziggy before and after the move and comparing to the
  pointer address of getCoolbearCBList( cblist, 0) after loop).
