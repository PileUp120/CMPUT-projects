/*
  ...
  tools you used:I swapped the position of unsigned int sign:SIGN with 
  unsigned int mantissa:MANTISSA. I compiled the program to see what 
  represantation was causing the assert failure, then I googled to search for
  a fix to the error, and I found an MS docs which I used in conjunction with
  a wikipedia article(both linked in the README.md) to figure out the solution
